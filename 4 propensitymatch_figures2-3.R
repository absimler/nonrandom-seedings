...
dens_road <- ggplot(sagesites, aes(x=distroad, color=Treatment_status)) +
  geom_density(aes(fill=Treatment_status), alpha=0.4) + 
  scale_color_manual(values=c("#CC3333", "#197bdd")) +
  scale_fill_manual(values=c("#CC3333", "#197bdd")) +
...
dens_ARTRpre <- ggplot(sagesites, aes(x=covTSF1pf, color=Treatment_status)) +
  ...
  scale_color_manual(values=c("#CC3333", "#197bdd")) +
  scale_fill_manual(values=c("#CC3333", "#197bdd")) +
  xlab("Estimated pre-fire sagebrush % cover") + mytheme
...
temp.gradient <- seq(from=min(sagesites$febaprtmean.tmean, na.rm=T), 
...
cfs.dens <- (pfcover2 + labs(tag="a") | postcover2 + labs(tag="b")) / 
